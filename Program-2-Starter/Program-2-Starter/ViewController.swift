...
class ViewController: UIViewController , ColorTableViewControllerDelegate{
    ...
    @IBOutlet weak var hslValueLabel: UILabel!
    @IBOutlet weak var hsvValueLabel: UILabel!
    /*
    ...
    */
    ...
    func ColorTableViewChooseColor(controller: ColorTableViewController, chosenColor color: String){
        ...
        colorViewerLabel.textColor = labelColor
        ...
        rgbValueLabel.text = "R:\(r) G:\(g) B:\(b)"
        
        let hsv = myColors.fetchHSV(color)
        let h = Float(hsv.H)
        let s = Float(hsv.S)
        let v = Float(hsv.V)
        hsvValueLabel.text = "H:\(h) S:\(s) V:\(v)"
        
        let hsl = myColors.fetchHSL(color)
        let h2 = Float(hsl.H)
        let s2 = Float(hsl.S)
        let v2 = Float(hsl.L)
        hslValueLabel.text = "H:\(h2) S:\(s2) L:\(v2)"
