...
class ViewController: UIViewController {
    ...
    func matchColorLogic() -> Bool{
        ...
        //Randomly pick what number button will hold the correct answer
        ...
        //The other two buttons are random colors
        var prev = self.myColors.fetchRandomColor()
        var next = self.myColors.fetchRandomColor()
        
        if(buttonAnswer == 0){
            btnAnswer1OUTLET.setTitle(colorName, forState: UIControlState.Normal)
        ...
        if(buttonAnswer == 1){
            ...
            btnAnswer2OUTLET.setTitle(colorName, forState: UIControlState.Normal)
            btnAnswer3OUTLET.setTitle(next.colorName, forState: UIControlState.Normal)
            button2Correct = true
        }
        ...
        if(buttonAnswer == 2){
            ...
            btnAnswer2OUTLET.setTitle(prev.colorName, forState: UIControlState.Normal)
            btnAnswer3OUTLET.setTitle(colorName, forState: UIControlState.Normal)
            button3Correct = true
        }
    ...
    //Update the square color for the user to guess
    func changeColor() -> Bool{
        //Grab a new color from the Colors tuple array
        var randColor = self.myColors.fetchRandomColor()
        
        //Print out the right answer in the output
        println("\(randColor.hexValue) : \(randColor.colorName) ")
        
        //Set the color name so it can be put on a button
        colorName = randColor.colorName
        
        //Get the UIColor from the new randColor's hexValue
        var uicolor = self.myColors.hexStringToUIColor(randColor.hexValue)
